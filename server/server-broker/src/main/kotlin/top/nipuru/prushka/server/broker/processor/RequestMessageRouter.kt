    ...
    private fun route(message: RequestMessageContainer): BrokerClientItem? {
        return when (message) {
            is AuthServerRequest -> {
                val name = top.nipuru.prushka.server.common.ClientType.AUTH
                Broker.getClient(name)
            }

            is DatabaseServerRequest -> {
                val name = String.format("%s-%d", top.nipuru.prushka.server.common.ClientType.DB, message.dbId)
                Broker.getClient(name)
            }

            is SharedServerRequest -> {
                val name = top.nipuru.prushka.server.common.ClientType.SHARED
                Broker.getClient(name)
            }
            ...
            is LogServerRequest -> {
                val name = top.nipuru.prushka.server.common.ClientType.LOG
                Broker.getClient(name)
            }
