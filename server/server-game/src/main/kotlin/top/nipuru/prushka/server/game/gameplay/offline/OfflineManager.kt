package top.nipuru.prushka.server.game.gameplay.offline
...
class OfflineManager(player: GamePlayer) : BaseManager(player) {
    ...
    private val offlineDataMessageQueue = LinkedList<top.nipuru.prushka.server.common.message.PlayerOfflineDataMessage>()
    ...
    fun pushOfflineData(name: String, playerId: Int, dbId: Int, moduleName: String, data: String) {
        val message =
            top.nipuru.prushka.server.common.message.PlayerOfflineDataMessage(name, playerId, dbId, moduleName, data)
        // 并不立即发送，而是等到下一tick
