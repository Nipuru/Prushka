package top.nipuru.prushka.server.auth
...
import top.nipuru.prushka.server.auth.config.Config
import top.nipuru.prushka.server.auth.config.loadConfig
import top.nipuru.prushka.server.auth.database.DatabaseFactory
import top.nipuru.prushka.server.auth.http.rootRouting
import top.nipuru.prushka.server.auth.logger.logger
import top.nipuru.prushka.server.auth.processor.PlayerRequestHandler
import top.nipuru.prushka.server.auth.processor.connection.CloseEventAuthProcessor
import top.nipuru.prushka.server.auth.util.JWTUtil
import top.nipuru.prushka.server.auth.util.overdue
...
object AuthServer {
    ...
    private fun initHttpServer() {
        httpServer = embeddedServer(Netty, 11300) {
            ...
            install(CallLogging) {
                ...
                logger = top.nipuru.prushka.server.auth.logger.logger
            }
    ...
    private fun initBrokerClient(config: Config) {
        try {
            ...
            builder.name(top.nipuru.prushka.server.common.ClientType.AUTH)
            builder.type(top.nipuru.prushka.server.common.ClientType.AUTH)
