...
class FriendCommand : CommandTree {
    ...
    private suspend fun add(context: CommandContext<CommandSourceStack>) {
    ...
    private suspend fun remove(context: CommandContext<CommandSourceStack>) {
        val player = context.source.gamePlayer
        val target = context.getFutureArgument<PlayerInfoMessage?>("player_name")
        if (target == null) {
            MessageType.FAILED.sendMessage(player, "玩家不存在")
    ...
    private suspend fun accept(context: CommandContext<CommandSourceStack>) {
        val player = context.source.gamePlayer
        val target = context.getFutureArgument<PlayerInfoMessage?>("player_name")
        if (target == null) {
            MessageType.FAILED.sendMessage(player, "玩家不存在")
    ...
    private suspend fun reject(context: CommandContext<CommandSourceStack>) {
        val player = context.source.gamePlayer
        val target = context.getFutureArgument<PlayerInfoMessage?>("player_name")
        if (target == null) {
            MessageType.FAILED.sendMessage(player, "玩家不存在")
